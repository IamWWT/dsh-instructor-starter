#!/usr/bin/env bash
# ============================================================================
# install-dsh.sh — 把 dsh-instructor-starter 装入 DSH（DeepSeek Harness）
#
# 装入内容：
#   1. agent preset：presets/instructor/ → $DSH_HOME/.agent-presets/instructor/
#      （agent.cordis.yml + preset.yml；DSH 每次读 roster 时重新发现，无需重启）
#   2. 随包 skills：presets/instructor/skills/（presentation-craft/voice-video/rehearsal/competition-defense）
#      随 preset 一起复制、由 preset 的 skill-filesystem 行以 preset 层（scoped）
#      注册——讲师模式内 shadow 全局同名技能，模式外不影响。★ 因此本脚本
...
#   - 安装后新开一个 DSH 会话（选择"讲师模式"preset）即可生效；
#     正在运行的会话保持旧 persona，不需要也不应该重启 3082。
# ============================================================================
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PRESET_SRC="$ROOT/presets/instructor"
...
echo "== dsh-instructor-starter 安装 (mode=$MODE) =="
echo "  DSH_HOME = $DSH_HOME"

if [ "$MODE" = "uninstall" ]; then
    PRESET_DST="$DSH_HOME/.agent-presets/instructor"
    if [ -e "$PRESET_DST" ]; then
        mv "$PRESET_DST" "${PRESET_DST}.bak-$TS"
        echo "  移除 preset: instructor → .bak-$TS"
    else
        echo "  preset 不存在，跳过"
    fi
...
echo "== 1/1 agent preset（含 skills/）→ $DSH_HOME/.agent-presets/instructor =="
PRESET_DST="$DSH_HOME/.agent-presets/instructor"
if [ -e "$PRESET_DST" ]; then
    mv "$PRESET_DST" "$PRESET_DST.bak-$TS"
    echo "  备份: instructor → instructor.bak-$TS"
fi
mkdir -p "$PRESET_DST"
...
echo ""
echo "✅ 安装完成。"
echo "  - 新会话选择『讲师模式』preset 即生效（DSH 每次读 roster 重新发现，无需重启）。"
echo "  - 随包 skills（presentation-craft/voice-video/rehearsal/competition-defense）以 preset 层注册，讲师模式内 shadow 全局；"
